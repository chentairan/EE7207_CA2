...
def plot_xt(fuzzy, mpc, opt, T, title_name, axis_name):
...
def plot_xy(fuzzy_x, fuzzy_y, mpc_x, mpc_y, opt_x, opt_y):
...
if __name__ == "__main__":
    ...
    for i, init_state in enumerate(init_states):
        if (run_code):
            ...
            # set data
            T = opt["T"]
            fuzzy_x, mpc_x, opt_x = fuzzy["fuzzy_x"], mpc["mpc_x"], opt["opt_x"]
            fuzzy_y, mpc_y, opt_y = fuzzy["fuzzy_y"], mpc["mpc_y"], opt["opt_y"]
            fuzzy_theta, mpc_theta, opt_theta = fuzzy["fuzzy_theta"], mpc["mpc_theta"], opt["opt_theta"]
            fuzzy_u, mpc_u, opt_u = fuzzy["fuzzy_u"], mpc["mpc_u"], opt["opt_u"]

        # plot results
        fig = plt.figure(figsize=(12.8, 9.6), dpi=326)
        fig.suptitle(f'Initial state: x = {init_state[0]}, y = {init_state[1]}, $\\theta$ = {init_state[2]}')
        plt.subplot(2, 2, 1)
        plot_xy(fuzzy_x, fuzzy_y, mpc_x, mpc_y, opt_x, opt_y)
        plt.subplot(2, 2, 2)
        plot_xt(fuzzy_y, mpc_y, opt_y, T, '$y-t$', 'y/m')
        plt.subplot(2, 2, 3)
        plot_xt(fuzzy_theta, mpc_theta, opt_theta, T, '$\\theta-t$', '$\\theta/degree$')
        plt.subplot(2, 2, 4)
        plot_xt(fuzzy_u, mpc_u, opt_u, T, '$u-t$', 'u/degree')
